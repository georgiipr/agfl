...
def train_eval_eeg(model, train_loader, val_loader, device, epochs=100, lr=3e-4):
    ...
    scaler = torch.cuda.amp.GradScaler(device_type, enabled=(device_type == 'cuda'))
    ...
    print(f"Final Results -> Accuracy: {acc:.4f} | F1 (Macro): {f1:.4f} | ROC-AUC: {auc:.4f} |loss: {avg_loss:.4f}")
