...
class StableTransformerBlock(nn.Module):
	...
	# self.norm1 = nn.RMSNorm(dim, eps=1e-6) This line is not working, so I changed it 
        self.norm1 = self.make_norm(dim, eps=1e-6)
        ...
        # self.norm2 = nn.RMSNorm(dim, eps=1e-6)   This line is not working, so I changed it 
        self.norm2 = self.make_norm(dim, eps=1e-6)
    ...
    # The following code is added to resolve the issue with unavailability of RSMNorm	
    def make_norm(self, dim, eps=1e-6):
        if hasattr(nn, "RMSNorm"):
            return nn.RMSNorm(dim, eps=eps)
        
        norm = nn.LayerNorm(dim, eps=eps)
        # Fix bias (beta) to 0
